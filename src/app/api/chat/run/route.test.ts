...
import { GlobalChatError } from "@/lib/chat/global-chat-client";
...
vi.mock("@/lib/chat/global-chat-client", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/chat/global-chat-client")>();
  return { ...actual, streamGlobalChat: mocks.streamGlobalChat };
});
...
beforeEach(() => {
  ...
  mocks.streamGlobalChat.mockReset();
  ...
  mocks.streamGlobalChat.mockImplementation(async function* () {
    yield "Hola";
  });
});
...
describe("POST /api/chat/run", () => {
  ...
  it("streams accumulated internal content from the reconstructed branch", async () => {
    ...
    expect(mocks.streamGlobalChat).toHaveBeenCalledWith({
      messages: [
  ...
  it("returns a safe internal SSE error for a typed chat failure", async () => {
    mocks.streamGlobalChat.mockImplementation(async function* () {
      ...
      throw new GlobalChatError("CHAT_RATE_LIMITED");
    });
